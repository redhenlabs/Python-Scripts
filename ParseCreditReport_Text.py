...
"""
"""
This reads all text files ( in this case DTI credit history files provided by credit bureau ),
...
SUBJECT_VAL,
SNN_VAL,
BIRTHDT_VAL,AKA_VAL,CURRADD_VAL,DTREPORT_VAL,
FORMR_VAL,SUBCODE_COLLECTION,ECOA_COLLECTION ,OPENED_COLLECTION,CLOSED_COLLECTION,
PLACED_COLLECTION,
CREDITOR_COLLECTION,
MOP_COLLECTION, 
ACCOUNT_COLLECTION,
...
dirname = "<Input directories for credit reports>/"
outdirname = "<Output directories for analysis  reports>"
csvfilename="<report file name >.csv"
...
#                                print("\n")
        ...
        fr.close
        #print (len(SUBNAME), len(SUBCODE),SUBCODE)
 
        for i in range(1,len(SUBNAME_COLLECTION)):
#            print()
          writer.writerow((filename,Report_Number,Borrower_Coborrower,FOR_VAL,SUBNAME_VAL,INFILE_VAL,DATE_VAL,TIME_VAL,SUBJECT_VAL,SNN_VAL,BIRTHDT_VAL,AKA_VAL,CURRADD_VAL,DTREPORT_VAL,FORMR_VAL,''.join(map(str,SUBNAME_COLLECTION[i])),''.join(map(str,SUBCODE_COLLECTION[i])),''.join(map(str, ECOA_COLLECTION[i])) ,''.join(map(str,OPENED_COLLECTION [i])),''.join(map(str,CLOSED_COLLECTION[i])),''.join(map(str,PLACED_COLLECTION[i])),''.join(map(str,CREDITOR_COLLECTION[i])),''.join(map(str,MOP_COLLECTION[i]))  ,''.join(map(str, ACCOUNT_COLLECTION[i]))  ,''.join(map(str, VERIFIED_COLLECTION[i])) ,''.join(map(str, BALANCE_COLLECTION[i])),''.join(map(str,REMARKS_COLLECTION[i]))))
#          writer.writerow((filename,Report_Number,Borrower_Coborrower,FOR_VAL,SUBNAME_VAL,INFILE_VAL,DATE_VAL,TIME_VAL,SUBJECT_VAL,SNN_VAL,BIRTHDT_VAL,AKA_VAL,CURRADD_VAL,DTREPORT_VAL,FORMR_VAL,''.join(map(str,SUBNAME[i])),''.join(map(str,SUBCODE[i])),''.join(map(str, ECOA[i])),''.join(map(str,OPENED [i])),''.join(map(str,CLOSED[i])),''.join(map(str,PLACED[i])),''.join(map(str,CREDITOR[i])),''.join(map(str,MOP[i])),''.join(map(str, ACCOUNT[i])),''.join(map(str, VERFIED[i])),''.join(map(str, BALANCE[i])),''.join(map(str,REMARKS[i]))))
#          print((filename,Report_Number,Borrower_Coborrower,FOR_VAL,SUBNAME_VAL,INFILE_VAL,DATE_VAL,TIME_VAL,SUBJECT_VAL,SNN_VAL,BIRTHDT_VAL,AKA_VAL,CURRADD_VAL,DTREPORT_VAL,FORMR_VAL,''.join(map(str,SUBNAME[i]))))
