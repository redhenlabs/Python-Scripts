# -*- coding: utf-8 -*-
"""
for transforming text based credit reports to CSV 
...
dirname = "<Your input directory name>/"
outdirname = "<your output directory name>/"
csvfilename="<your report file name>.csv"
...
#      writer.writerow(('Inbox','MailBox','From_Name','Manual_Date','date','day','month','year','MMYYYY','From','To','CC','CC1','BCC','Subject','FULL_Content'))   
      for filename in self.examFileName:
        ...
        SUBNAME_COLLECTION=np.array([""])
        SUBCODE_COLLECTION=np.array([""])
        ECOA_COLLECTION=np.array([""])
        OPENED_COLLECTION =np.array([""])
        CLOSED_COLLECTION =np.array([""])
        PLACED_COLLECTION=np.array([""])
        CREDITOR_COLLECTION=np.array([""])
        MOP_COLLECTION=np.array([""])
        ACCOUNT_COLLECTION=np.array([""])
        VERIFIED_COLLECTION=np.array([""])
        BALANCE_COLLECTION=np.array([""])
        REMARKS_COLLECTION=np.array([""])       
...
#                             print (line.index(SUBNAME_split[i]),SUBNAME_split[i]+' '+SUBNAME_split[i+1] )
                             ...
                             ALL_COL_NAME.append(SUBNAME_split[i]+"_1-12")
#                             print("PayPAT 1 :",SUBNAME_split[i])
                             j = i +1
                             #print(i,j)
                         elif (i != j):
...
#                             print (line.index(ACCOUNT_split[i]),ACCOUNT_split[i]+' '+ACCOUNT_split[i+1] )
                             ...
                             ALL_COL_NAME.append(ACCOUNT_split[i]+"_13-24")
#                             print("PayPAT 2 :",ACCOUNT_split[i])
                             j = i +1
                             #print(i,j)
                         elif (i != j):
            ...
            for num1, line in enumerate(fr, 1):
                ...
                if (num1 == SUBNAMELineNumber_Collection ):
                    ...
                    for i in range(len(SUBNAME_Collection_split)):
                         ...
                         ALL_COL_NAME.append(SUBNAME_Collection_split[i]+'_COLLECTION')
#                        
...
#                       print("i=",i,ACCOUNT_Colllection_split[i],line.index(ACCOUNT_Colllection_split[i]))
                       ...
                       ALL_COL_NAME.append(ACCOUNT_Colllection_split[i]+'_COLLECTION')
...
#        print(ALL_COL_NAME)
        ...
        if file_count ==0 :
            ...
            print("Dont print Header" )#- File Number :",file_count)
...
#                                print ('SUBNAME :' ,line[SUBNAME_COL_POS_Collection[wordpos]:SUBNAME_COL_POS_Collection[wordpos+1]])
                                SUBNAME_COLLECTION = np.vstack([SUBNAME_COLLECTION,(line[SUBNAME_COL_POS_Collection[wordpos]:SUBNAME_COL_POS_Collection[wordpos+1]])])
...
#                                print ('SUBCODE :' ,line[SUBNAME_COL_POS_Collection[wordpos]:SUBNAME_COL_POS_Collection[wordpos+1]])
                                SUBCODE_COLLECTION = np.vstack([SUBCODE_COLLECTION,(line[SUBNAME_COL_POS_Collection[wordpos]:SUBNAME_COL_POS_Collection[wordpos+1]])])
...
#                                print ('ECOA :',line[SUBNAME_COL_POS_Collection[wordpos]:SUBNAME_COL_POS_Collection[wordpos+1]])
                                ECOA_COLLECTION= np.vstack([ECOA_COLLECTION,(line[SUBNAME_COL_POS_Collection[wordpos]:SUBNAME_COL_POS_Collection[wordpos+1]])])
...
#                                print ('OPENED :',line[SUBNAME_COL_POS_Collection[wordpos]:SUBNAME_COL_POS_Collection[wordpos+1]])
                                OPENED_COLLECTION= np.vstack([OPENED_COLLECTION,(line[SUBNAME_COL_POS_Collection[wordpos]:SUBNAME_COL_POS_Collection[wordpos+1]])])
...
#                                print ('CLOSED:',line[SUBNAME_COL_POS_Collection[wordpos]:SUBNAME_COL_POS_Collection[wordpos+1]])
                                CLOSED_COLLECTION= np.vstack([CLOSED_COLLECTION,(line[SUBNAME_COL_POS_Collection[wordpos]:SUBNAME_COL_POS_Collection[wordpos+1]])])
...
#                                print ('$PLACED :' ,line[SUBNAME_COL_POS_Collection[wordpos]:SUBNAME_COL_POS_Collection[wordpos+1]])
                                PLACED_COLLECTION= np.vstack([PLACED_COLLECTION,(line[SUBNAME_COL_POS_Collection[wordpos]:SUBNAME_COL_POS_Collection[wordpos+1]])])
...
#                                print ('CREDITOR :' ,line[SUBNAME_COL_POS_Collection[wordpos]:SUBNAME_COL_POS_Collection[wordpos+1]])
                                CREDITOR_COLLECTION= np.vstack([CREDITOR_COLLECTION,(line[SUBNAME_COL_POS_Collection[wordpos]:SUBNAME_COL_POS_Collection[wordpos+1]])])
                              if SUBNAME_COL_NAME_Collection[wordpos] == 'MOP':
                                line = line.replace(" ","_")
                                line = line.replace("\n","")
                                line = line.replace("\r","")
#                                print ('MOP :',line[SUBNAME_COL_POS_Collection[wordpos]:])
                                MOP_COLLECTION= np.vstack([MOP_COLLECTION,(line[SUBNAME_COL_POS_Collection[wordpos]:])])
#                                print("\n")
...
#                                print ('ACCOUNT :' ,line[ACCOUNT_COL_POS_Collection[wordpos]:ACCOUNT_COL_POS_Collection[wordpos+1]])
                                ACCOUNT_COLLECTION = np.vstack([ACCOUNT_COLLECTION,(line[ACCOUNT_COL_POS_Collection[wordpos]:ACCOUNT_COL_POS_Collection[wordpos+1]])])
...
#                                print (num1,'VERIFIED :' ,line[ACCOUNT_COL_POS_Collection[wordpos]:ACCOUNT_COL_POS_Collection[wordpos+1]])
                                VERIFIED_COLLECTION = np.vstack([VERIFIED_COLLECTION,(line[ACCOUNT_COL_POS_Collection[wordpos]:ACCOUNT_COL_POS_Collection[wordpos+1]])])
...
#                                print ('BALANCE :' ,line[ACCOUNT_COL_POS_Collection[wordpos]:ACCOUNT_COL_POS_Collection[wordpos+1]])
                                BALANCE_COLLECTION = np.vstack([BALANCE_COLLECTION,(line[ACCOUNT_COL_POS_Collection[wordpos]:ACCOUNT_COL_POS_Collection[wordpos+1]])])
                              if ACCOUNT_COL_NAME_Collection[wordpos] == 'REMARKS':
                                line = line.replace(" ","_")
                                line = line.replace("\n","")
                                line = line.replace("\r","") 
#                                print ('REMARKS :' ,line[ACCOUNT_COL_POS_Collection[wordpos]:])
                                REMARKS_COLLECTION = np.vstack([REMARKS_COLLECTION,(line[ACCOUNT_COL_POS_Collection[wordpos]:])])
#                                print("\n")
...
#        print(len(MO), len(SUBNAME_COLLECTION))
        traverse =0
        if len(MO) > len(SUBNAME_COLLECTION):
            traverse = len(MO)
        else :
            traverse = len(SUBNAME_COLLECTION)
           
        print("Rows :",traverse )
#        for i in range(1,len(MO)):
        for i in range(1,traverse):
            #print(1)
#         writer.writerow((ECOA[i],COLLATRL_LOANTYPE[i],CLSD_PD[i],BALANCE[i],REMARKS[i],MO[i],N30_60_90[i]))
         writer.writerow((filename,Report_Number,Borrower_Coborrower,FOR_VAL,SUBNAME_VAL,INFILE_VAL,DATE_VAL,TIME_VAL,SUBJECT_VAL,SNN_VAL,BIRTHDT_VAL,AKA_VAL,CURRADD_VAL,DTREPORT_VAL,FORMR_VAL
         ,( ''.join(map(str,SUBNAME[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str,SUBCODE[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str,OPENED [i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str,HIGHCRED[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str,TERMS[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str,MAXDELQ[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str,PAYPAT112[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str,MOP[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str, ACCOUNT[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str, VERFIED[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str, CREDLIM[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str, PASTDUE[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str, AMT_MOP[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str, PAYPAT1324[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str, ECOA[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str,COLLATRL_LOANTYPE[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str,CLSD_PD[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str,BALANCE[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str,REMARKS[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str,MO[i])) if i <= len(MO)-1 else '')
         ,( ''.join(map(str,N30_60_90[i])) if i <= len(MO)-1 else '')
 
          ###########  COLLECTION RELATED ITEMS ################
         ,( ''.join(map(str,SUBNAME_COLLECTION[i])) if i <= len(SUBNAME_COLLECTION)-1 else '')
         ,( ''.join(map(str,SUBCODE_COLLECTION[i]))  if i <= len(SUBNAME_COLLECTION)-1 else '')
         ,( ''.join(map(str, ECOA_COLLECTION[i]))  if i <= len(SUBNAME_COLLECTION)-1 else '')
         ,( ''.join(map(str,OPENED_COLLECTION [i])) if i <= len(SUBNAME_COLLECTION)-1 else '')
         ,( ''.join(map(str,CLOSED_COLLECTION[i]))  if i <= len(SUBNAME_COLLECTION)-1 else '')
         ,( ''.join(map(str,PLACED_COLLECTION[i]))  if i <= len(SUBNAME_COLLECTION)-1 else '')
         ,( ''.join(map(str,CREDITOR_COLLECTION[i]))  if i <= len(SUBNAME_COLLECTION)-1 else '')
         ,( ''.join(map(str,MOP_COLLECTION[i]))  if i <= len(SUBNAME_COLLECTION)-1 else '')
         ,( ''.join(map(str,ACCOUNT_COLLECTION[i]))  if i <= len(SUBNAME_COLLECTION)-1 else '')
         ,( ''.join(map(str,VERIFIED_COLLECTION[i]))  if i <= len(SUBNAME_COLLECTION)-1 else '')
         ,( ''.join(map(str,BALANCE_COLLECTION[i]))  if i <= len(SUBNAME_COLLECTION)-1 else '')
         ,( ''.join(map(str,REMARKS_COLLECTION[i]))  if i <= len(SUBNAME_COLLECTION)-1 else '')
